...
const bookCreateValidator = z.object({
...
export default function BookCreate() {
  ...
  const { createBook, isSavingBook } = useBook();
  const navigate = useNavigate();

  const form = useForm({
    resolver: zodResolver(bookCreateValidator),
  });
  ...
  return (
    <>
      <PageHeader subTitle="Preencha as informações abaixo para criar um novo livro...">
        Novo Livro
      </PageHeader>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-5">
        ...
        <footer className="flex justify-end">
          <Button variation="primary" className="w-60" disabled={isSavingBook}>
            {!isSavingBook ? 'Cadastrar Livro' : 'Salvando Livro...'}
          </Button>
        </footer>
      </form>
    </>
